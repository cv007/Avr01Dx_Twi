...
startWrite      () { TWI0.MADDR &= ~RW; }   //reuse existing address
                static void     
dataW           (u8 v) { TWI0.MDATA = v; }
                static u8       
dataR           () { return TWI0.MDATA; }
                static u8       
...
ISR             (TWI0_TWIM_vect) 
    ...
    //==========
                ...
                void        
twim_callback   (twim_callbackT cb) { isrFuncCallback_ = cb; }
                void
twim_address    (u8 v) { address(v); } //twi is off after setting address
                void        
twim_off        () { off(); }
                void        
twim_on         () { off(); toStateIdle(); on(); }
                bool       
twim_isBusy     () { return isBusy(); } //if irq on, is busy
                bool        
twim_lastResultOK() { return lastResult_; }
                //set default or alternate pins
                void        
twim_defaultPins() { TWI_PULL_DEFAULT(); TWI_PORTMUX_DEFAULT(); }
                void        
twim_altPins    () { TWI_PULL_ALT(); TWI_PORTMUX_ALT(); }



                //write+read (or write only, or read only)
                void        
twim_writeRead  (const u8* wbuf, u16 wn, u8* rbuf, u16 rn) 
                {
                ...
                void        
twim_writeWrite (const u8* wbuf, u16 wn, const u8* wbuf2, u16 wn2) 
                {
                txbuf_ = wbuf; txbufEnd_ = &wbuf[wn];
                ...
                //write only alias
                void        
twim_write      (const u8* wbuf, u16 wn) { twim_writeRead( wbuf, wn, 0, 0); }

                //read only alias
                void
twim_read       (u8* rbuf, u16 rn) { twim_writeRead( 0, 0, rbuf, rn); }
