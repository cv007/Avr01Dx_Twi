                ...
                /*------------------------------------------------------------------------------
                    ...
                    bool myCallback(twis_irqstate_t state, u8 statusReg){
                ...
                typedef enum { 
                    TWIS_ADDRESSED, TWIS_MREAD, TWIS_MWRITE, TWIS_STOPPED, TWIS_ERROR 
                    } twis_irqstate_t;

                typedef bool(*twis_callback_t)(twis_irqstate_t state, u8 statusReg);

                void 
twis0_on        (u8 SlaveAddress, twis_callback_t callbackFunction);
                void 
twis0_off       ();
                void 
twis0_write     (u8 value);
                u8  
twis0_read      ();
                u8   
twis0_lastAddress();
                void 
twis0_address2  (u8 SlaveAddress2);
                void 
twis0_addressMask(u8 SlaveAddressMask); //no 2nd address when using this option
